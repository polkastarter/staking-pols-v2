...
const stakeAmount: bigint = 1000n * DECMULBN; // 1000 token
...
export function shouldBehaveLikeStakeV1(_timePeriod: number): void {
  ...
  describe("PolsStake : " + filenameHeader, function () {
    ...
    it("send 1000 reward tokens from admin account to staking contract", async function () {
      ...
      const tx = await this.rewardToken.connect(this.signers.admin).transfer(this.stake, amount);
      await tx.wait();

      const balance = await this.rewardToken.balanceOf(this.stake);
      console.log(
    ...
    it("decrease lock time period - setLockTimePeriod()", async function () {
      const lockTimePeriod: number = Number(await this.stake.lockTimePeriod());
    ...
    */
    ...
    it("setRewardToken()", async function () {
      const tx = await this.stake.connect(this.signers.admin).setRewardToken(this.rewardToken);
      await tx.wait();
  ...
  describe("test stake & unstake, time lock and rewards", function () {
    ...
    it("user can stake token", async function () {
      console.log("staking now ... stakeAmount =", ethers.formatUnits(stakeAmount, stakeTokenDecimals));
      ...
      expect(await this.stakeToken.balanceOf(this.signers.user1)).to.equal(
        user1BalanceStart - stakeAmount,
        "user1 balance was not reduced by staked amount",
      );
    });

    it("verify getUnlockTime_msgSender()", async function () {
      const unlockTime = Number(await this.stake.connect(this.signers.user1).getUnlockTime_msgSender());
      const stakeTime = Number(await this.stake.connect(this.signers.user1).stakeTime_msgSender());
    ...
    it("user should have claimable rewards after staking for some time", async function () {
      ...
      const userClaimableRewards_expected = stakeAmount * BigInt(blockTime - stakeTime1);
    ...
    it("user can stake same amount again, should have staked 2x then", async function () {
      ...
      stakeBalance = await this.stake.stakeAmount(this.signers.user1);
      console.log("stakeBalance =", ethers.formatUnits(stakeBalance, stakeTokenDecimals));
      expect(stakeBalance).to.equal(stakeAmount * 2n, "stake contract does not reflect staked amount");

      expect(await this.stakeToken.balanceOf(this.signers.user1)).to.equal(
        user1BalanceStart - (stakeAmount) - (stakeAmount),
        "user1 balance was not reduced by staked amount",
      );
    });
    ...
    it("after the 2nd staking, claimable rewards should have become accumulated reward", async function () {
      ...
      const blockTime = await blockTimestamp();
      const userAccumulatedRewards_expected = stakeAmount * BigInt(blockTime - stakeTime1);

      const userAccumulatedRewards_contract = await this.stake
        .connect(this.signers.user1)
        .userAccumulatedRewards_msgSender();

      difference = abs(userAccumulatedRewards_contract - userAccumulatedRewards_expected) / stakeAmount; // relative error to stakeBalance
    ...
    it("check userClaimableRewards", async function () {
       ...
       */
      const blockTime = await blockTimestamp();
      const userClaimableRewards_expected = stakeAmount * 2n * BigInt(blockTime - stakeTime2);
      console.log("userClaimableRewards_expected =", userClaimableRewards_expected.toString());
    ...
    it("user can unstake after the lockTimePeriod is over", async function () {
      ...
      const tx = await this.stake.connect(this.signers.user1).withdraw(lastStakeBalance / 4n);
      await tx.wait();
      ...
      expectedRewards = stakeAmount * BigInt(stakeTime2 - stakeTime1) + (stakeAmount * 2n * BigInt(blocktime - stakeTime2));
       ...
       * Check userAccumulatedRewards
       */
      // const rewardsStake1 = stakeAmount * (15) * (timePeriod); // TODO - use measured, expired time
      // const rewardsStake2 = stakeAmount * (10) * (timePeriod);
      // const userAccumulatedRewards_expected = rewardsStake1 + (rewardsStake2);

      const userAccumulatedRewards_expected = expectedRewards; // stakeAmount * (stakeTime2 - stakeTime1) + ( stakeAmount * (2) * (blocktime - stakeTime2) );
    ...
    it("admin can disable reward token and will receive all reward tokens left", async function () {
      ...
      const tx = await this.stake.connect(this.signers.admin).setRewardToken(ethers.ZeroAddress);
      await tx.wait();
