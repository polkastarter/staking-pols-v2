...
import { timePeriod, timeToPeriod, getTimestamp, moveTime, waitTime, setTime, logStringTime /* logCurrentTimeRelative */ } from "../libs/BlockTimeHelper";
...
export function basicTestsV2(
    ...
    let userTotalRewards_contract = 0n; // typeof bigint; // causes problems with solidity-coverage
    ...
    describe("PolsStakeV2 : " + filenameHeader, function () {
        ...
        /*
            it("test setLockTimePeriodOptions() - ethers TEST ***********************************", async function () {
              ...
              const tx2 = await this.stakeV2
                ...
                .setLockTimePeriodOptions(lockTimePeriods, lockTimePeriodRewardFactors);
              await tx2.wait();
    ...
    describe("test stake & unstake, time lock and rewards", function () {
        ...
        it("user should have total rewards after staking for 1 time period", async function () {
            ...
            const userTotalRewards_expected = STAKE_AMOUNT * (rewardTime - BigInt(timeStake1)) * lockTimePeriodRewardFactor / REWARDS_DIV;

            console.log("userTotalRewards_expected =", userTotalRewards_expected, " = POLSpower : ", formatRewards(userTotalRewards_expected));

            userTotalRewards_contract = await this.stakeV2.connect(this.signers.user1).userTotalRewards_msgSender();
            console.log("userTotalRewards_contract =", userTotalRewards_contract, " = POLSpower : ", formatRewards(userTotalRewards_contract));

            difference = absDiff(userTotalRewards_contract, userTotalRewards_expected) / stakeBalance;
            console.log("difference =", difference);
            expect(difference).to.lte(5, "userTotalRewards calculation is too far off");
        });
        ...
        // it("2nd stake - day 10 : check userTotalRewards for 14 days lock period", async function () {
        ...
        //     expect(userTotalRewards_contract).to.eq(
        ...
        it("day 10 : extend lock period by 7 days from now - using `extendLockTime` - ERROR", async function () {
            await moveTime(10 * DAYS);
            ...
            console.log("unlocktime_before =", unlocktime_before);

            let remainingLockPeriod = await this.stakeV2.connect(this.signers.user1).remainingLockPeriod_msgSender();
            console.log("remainingLockPeriod (days) =", timeToPeriod(remainingLockPeriod));
            ...
            const userTotalRewards_expected = userTotalRewards_contract_before + (BigInt(7 * DAYS) * stakeAmount);
            ...
            remainingLockPeriod = await this.stakeV2.connect(this.signers.user1).remainingLockPeriod_msgSender();
            expect(remainingLockPeriod).to.be.closeTo(10 * DAYS, 120);
        });
        ...
        it("day 12 : topUp stake amount", async function () {
            ...
            let userTotalRewards_contract = await this.stakeV2
                .connect(this.signers.user1)
                .userTotalRewards_msgSender();
            console_log_reward("userTotalRewards_contract =", userTotalRewards_contract);

            let remainingLockPeriod = await this.stakeV2
                .connect(this.signers.user1).remainingLockPeriod_msgSender();
            console.log("remainingLockPeriod =", remainingLockPeriod, Number(remainingLockPeriod) / DAYS);
            ...
            const userTotalRewards_expected = userTotalRewards_contract + (STAKE_AMOUNT * remainingLockPeriod);
            console_log_reward("userTotalRewards_expected =", userTotalRewards_expected);

            userTotalRewards_contract = await this.stakeV2.connect(this.signers.user1).userTotalRewards_msgSender();
            console_log_reward("userTotalRewards_contract =", userTotalRewards_contract);

            expect(userTotalRewards_contract).to.be.closeTo(
                userTotalRewards_expected,
                userTotalRewards_expected / 10000n,
            ); // allow 0.01% error

            const remainingLockPeriod_after = await this.stakeV2.connect(this.signers.user1).remainingLockPeriod_msgSender();
        ...
        it("user can not topUp stake amount with zero lock period after the lockTimePeriod is over", async function () {
            ...
            expect(await this.stakeV2.connect(this.signers.user1).remainingLockPeriod_msgSender()).to.eq(0);
        ...
        it("user can unstake after the lockTimePeriod is over", async function () {
             ...
             */
            const userTotalRewards_contract = await this.stakeV2.connect(this.signers.user1).userTotalRewards_msgSender();
            const userTotalRewards_expected = userTotalRewards + (stakeBalance * BigInt(5 * DAYS)) // lockTimePeriodRewardFactor / REWARDS_DIV
            difference = absDiff(userTotalRewards_expected, userTotalRewards_contract) / lastStakeBalance; // TODO check if this is right
            console.log("userTotalRewards       : difference contract vers expected =", difference);
            expect(userTotalRewards_contract).to.be.closeTo(userTotalRewards_expected, 1000, "userTotalRewards is too far off");
        });
